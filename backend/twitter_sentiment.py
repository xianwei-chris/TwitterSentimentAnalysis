...
def search_recent_tweets_as_df(query, tweet_fields):
    ...
    tweets = client.search_recent_tweets(
        query=query, tweet_fields=tweet_fields, max_results=100
    )
...
def text_sentiment(df):
    ...
    for text in df.text:
        ...
        sentiment_score_positive_list.append(
            sentiment_all["SentimentScore"]["Positive"]
        )
        sentiment_score_negative_list.append(
            sentiment_all["SentimentScore"]["Negative"]
        )
